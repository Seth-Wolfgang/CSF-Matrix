...
namespace IVSparse {
    ...
    void VCSC<T, indexT, columnMajor>::write(char* filename) {
        ...
        #pragma omp parallel sections
        {
            #pragma omp section
            {
                ASYNCH_WRITE(fp, metadata, META_DATA_SIZE, 0);
            }
            #pragma omp section
            {
                ASYNCH_WRITE(fp, valueSizes, offset, META_DATA_SIZE);
            }
            #pragma omp section
            {
                ASYNCH_WRITE(fp, indexSizes, offset, META_DATA_SIZE + offset);
            }
        }
        ...
        ASYNCH_WRITE(fp, values[i], valueSizes[i] * sizeof(T), tempOffset);
        ASYNCH_WRITE(fp, counts[i], valueSizes[i] * sizeof(indexT), tempCountsStart);
        ASYNCH_WRITE(fp, indices[i], indexSizes[i] * sizeof(indexT), tempIndicesStart);
